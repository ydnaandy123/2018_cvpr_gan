...
from module import generator_zero, discriminator_zero, high_light
import time

flags = tf.app.flags.FLAGS
tf.flags.DEFINE_string('mode', "test", "Mode train/ test-dev/ test")
tf.flags.DEFINE_boolean('debug', True, "Is debug mode or not")
tf.flags.DEFINE_string('dataset_dir', "./dataset/filter_bird", "directory of the dataset")
...
def main(args=None):
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope('Network'):
            ...
            logits_a = generator_zero(real_a, flags, False, name="Generator_A2B")
            segment_a = tf.nn.sigmoid(logits_a, name='segment_a')
