...
from module import generator_resnet_u, discriminator_wgangp, high_light
import time

flags = tf.app.flags.FLAGS
tf.flags.DEFINE_string('mode', "train", "Mode train/ test-dev/ test")
tf.flags.DEFINE_boolean('debug', True, "Is debug mode or not")
tf.flags.DEFINE_string('dataset_dir', "./dataset/dof2black_bird", "directory of the dataset")
...
tf.flags.DEFINE_integer("training_iter", 20000, "data size for training")
...
tf.flags.DEFINE_integer('observe_freq', 50, "observe training image every observe_freq iterations")
...
def main(args=None):
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope('Network'):
            ...
            segment_a = generator_resnet_u(real_a, flags, False, name="Generator_A2B")
            segment_a_test = generator_resnet_u(real_a_test, flags, True, name="Generator_A2B")
            ...
            dis_fake_b = discriminator_wgangp(fake_b_f, flags, reuse=False, name="Discriminator_B")
            dis_real_b = discriminator_wgangp(real_b_f, flags, reuse=True, name="Discriminator_B")
            ...
            with tf.name_scope('loss_dis_b'):
                ...
                loss_dis_b_adv_fake = tf.reduce_mean(dis_fake_b)
                loss_dis_b = tf.reduce_mean(dis_fake_b) - tf.reduce_mean(dis_real_b)
                with tf.name_scope('wgan-gp'):
                    alpha = tf.random_uniform(shape=[flags.batch_size, 1], minval=0., maxval=1.)
                    differences = fake_b_f - real_b_f
                    interpolates = real_b_f + (alpha * differences)
                    gradients = tf.gradients(
                        discriminator_wgangp(interpolates, flags, reuse=True, name="Discriminator_B"),
                        [interpolates])[0]
        ...
        with tf.Session(config=tfconfig) as sess:
            ...
            if flags.mode == 'train':
                ...
                while global_step_sess < flags.training_iter:
                    try:
                        _ = sess.run([train_op_dis_b], feed_dict=feed_dict_train)
                        _ = sess.run([train_op_gen_a2b], feed_dict=feed_dict_train)
                        ...
                        if flags.debug and global_step_sess % flags.observe_freq == 1:
                            ...
                            dataset_parser.visualize_data(
                                ...
                                segment_a=segment_a_sess, fake_b=fake_b_sess, shape=(3, 3),
