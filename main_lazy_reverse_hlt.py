...
def main(args=None):
    ...
    with tf.variable_scope('Network'):
        ...
        segment_a = generator_resnet_lazy_reverse(real_a, flags, False, name="Generator_A2B")
        # TODO : gp, argmax?
        ...
        # dis_fake_b = discriminator_wgangp(fake_b, flags, reuse=False, name="Discriminator_B")
        # dis_fake_b_pool = discriminator_wgangp(fake_b_pool, flags, reuse=True, name="Discriminator_B")
        # dis_real_b = discriminator_wgangp(real_b, flags, reuse=True, name="Discriminator_B")

        fake_b_f, fake_b_pool_f, real_b_f = \
            tf.reshape(fake_b, [-1]), tf.reshape(fake_b_pool, [-1]), tf.reshape(real_b, [-1])
        dis_fake_b = discriminator_wgangp_reverse(fake_b_f, flags, reuse=False, name="Discriminator_B")
        dis_fake_b_pool = discriminator_wgangp_reverse(fake_b_pool_f, flags, reuse=True, name="Discriminator_B")
        dis_real_b = discriminator_wgangp_reverse(real_b_f, flags, reuse=True, name="Discriminator_B")
        ...
        with tf.name_scope('loss_dis_b'):
            ...
            with tf.name_scope('wgan-gp'):
                ...
                gradients = tf.gradients(
                    discriminator_wgangp_reverse(interpolates, flags, reuse=True, name="Discriminator_B"),
                    [interpolates])[0]
