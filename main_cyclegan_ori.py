...
tf.flags.DEFINE_integer('observe_freq', 100, "observe training image every observe_freq iterations")
...
def main(args=None):
    ...
    if False:
        ...
        dataset_parser.data2record(name='hlt_train.tfrecords', set_type='train', test_num=None)
        dataset_parser.data2record(name='hlt_val.tfrecords', set_type='val', test_num=None)
    ...
    with tf.variable_scope('TFRecord'):
        with tf.variable_scope('DatasetA'):
            training_a_dataset = dataset_parser.tfrecord_get_dataset(
                name='horse_trainA.tfrecords', batch_size=flags.batch_size,
                shuffle_size=None)
            val_a_dataset = dataset_parser.tfrecord_get_dataset(
                name='horse_valA.tfrecords', batch_size=flags.batch_size)
        with tf.variable_scope('DatasetB'):
            training_b_dataset = dataset_parser.tfrecord_get_dataset(
                name='horse_trainB.tfrecords', batch_size=flags.batch_size,
                shuffle_size=None)
            val_b_dataset = dataset_parser.tfrecord_get_dataset(
                name='horse_valB.tfrecords', batch_size=flags.batch_size)
    ...
    with tf.variable_scope('Network'):
        ...
        with tf.control_dependencies(update_ops):
            train_op_gen_a2b = train_op(loss_gen_a2b, flags, trainable_var_gen_a2b, name='gen_a2b')
            train_op_dis_b = train_op(loss_dis_b, flags, trainable_var_dis_b, name='dis_b')
            train_op_gen_b2a = train_op(loss_gen_b2a, flags, trainable_var_gen_b2a, name='gen_b2a')
            train_op_dis_a = train_op(loss_dis_a, flags, trainable_var_dis_a, name='dis_a')
