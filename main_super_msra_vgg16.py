        ...
        with tf.name_scope('Network'):
            ...
            with tf.name_scope('Generator'):
                with slim.arg_scope(vgg.vgg_arg_scope()):
                    net, end_points = vgg.vgg_16(real_a - mean_rgb, num_classes=1000, is_training=True, spatial_squeeze=False)

                with tf.variable_scope('Generator_A2B'):
                    fuse_1 = tf.layers.conv2d_transpose(net, 512, [4, 4], strides=14, padding='same') + end_points['Network/Generator/vgg_16/pool4']
                    fuse_2 = tf.layers.conv2d_transpose(fuse_1, 256, [4, 4], strides=2, padding='same') + end_points['Network/Generator/vgg_16/pool3']
                    pred = tf.layers.conv2d_transpose(fuse_2, 1, [16, 16], strides=8, padding='same')
                    ...
                    segment_a = tf.nn.sigmoid(pred, name='segment_a')
            ...
            with tf.name_scope('loss_gen_a2b'):
                loss_gen_a2b = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
                    logits=pred, labels=real_b/255.0, name='sigmoid'), name='mean')
